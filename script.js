document.addEventListener('DOMContentLoaded', () => {
  ...
  if (propertyGrid && !document.getElementById('property-search-panel')) {
    ...
    panel.innerHTML = `
      ...
      <form class="property-search-form" id="property-search-form">
        <div class="property-search-field"><label for="type-filter">Property Type</label><select id="type-filter" name="type"><option value="all">All Types</option><option value="residential">Residential</option><option value="commercial">Commercial</option></select></div>
        <div class="property-search-field"><label for="location-filter">Location</label><select id="location-filter" name="location"><option value="all">All Locations</option><option value="al-thuqbah">Al-Thuqbah</option><option value="al-khobar">Al Khobar</option></select></div>
        <div class="property-search-field"><label for="status-filter">For</label><select id="status-filter" name="status"><option value="all">For Sale &amp; Rent</option><option value="sale">For Sale</option><option value="rent">For Rent</option></select></div>
        <div class="property-search-field"><label for="price-filter">Price Range</label><select id="price-filter" name="price"><option value="all">Any Price</option><option value="under1m">Under SAR 1M</option><option value="1to3m">SAR 1M – 3M</option><option value="3to5m">SAR 3M – 5M</option><option value="over5m">Over SAR 5M</option></select></div>
  ...
  const aiSprite = 'assets/ai-property-sprite.webp?v=3';
  const aiPositions = ['0% 0%', '50% 0%', '100% 0%', '0% 100%', '50% 100%', '100% 100%'];
  cards.forEach((card, index) => {
    ...
    card.innerHTML = `
      <div class="property-image" style="background-image:url('${aiSprite}');background-repeat:no-repeat;background-size:300% 200%;background-position:${aiPositions[index % aiPositions.length]};background-color:#102b42;">
        <img src="${item.image}" alt="${item.alt}" width="1200" height="800" loading="lazy" decoding="async" aria-hidden="true" style="opacity:0;position:absolute;inset:0;width:1px;height:1px;">
  ...
  const filterCards = () => {
    ...
    let visible = 0;
    cards.forEach(card => {
      const ok = (!q || card.textContent.toLowerCase().includes(q)) && (!status || status.value === 'all' || card.dataset.status === status.value) && (!type || type.value === 'all' || card.dataset.type === type.value) && (!location || location.value === 'all' || card.dataset.location === location.value) && priceMatch(card.dataset.price, price?.value);
      card.hidden = !ok;
      if (ok) visible++;
    });
    if (empty) empty.hidden = visible !== 0;
  };
  [search, status, type, location, price].forEach(el => { el?.addEventListener('input', filterCards); el?.addEventListener('change', filterCards); });
  document.getElementById('property-search-form')?.addEventListener('submit', event => { event.preventDefault(); filterCards(); propertyGrid?.scrollIntoView({ behavior:'smooth', block:'start' }); });
  ...
  document.querySelectorAll('.faq-question').forEach(button => button.addEventListener('click', () => { const item = button.closest('.faq-item'); const wasOpen = item?.classList.contains('open'); document.querySelectorAll('.faq-item').forEach(other => { other.classList.remove('open'); other.querySelector('.faq-question')?.setAttribute('aria-expanded','false'); }); if (!wasOpen && item) { item.classList.add('open'); button.setAttribute('aria-expanded','true'); } }));

  const heroActions = document.querySelector('.hero .hero-actions');
  if (heroActions) heroActions.innerHTML = `<a class="btn btn-light" href="#services">View Properties <i data-lucide="arrow-up-right"></i></a><a class="btn btn-outline-light" href="#advisor">Talk to an Advisor <i data-lucide="user-round"></i></a>`;
  const locationSection = document.getElementById('location');
  if (locationSection && !document.getElementById('advisor')) {
    const ctaSection = document.createElement('section'); ctaSection.className='section advisor-cta-section'; ctaSection.id='advisor';
    ctaSection.innerHTML = `<div class="container"><div class="advisor-cta-header"><span class="eyebrow">Explore &amp; Connect</span><h2>Find your next <em>property.</em></h2><p>Explore our property options or speak directly with an advisor for guidance tailored to your needs.</p></div><div class="advisor-cta-grid"><article class="advisor-cta-card"><div class="advisor-cta-image"><img src="assets/view-properties-generated.svg" alt="Contemporary luxury property" loading="lazy" decoding="async"></div><div class="advisor-cta-content"><span class="property-type">01 · Properties</span><h3>View Properties</h3><p>Browse the available property information and discover spaces that fit your plans.</p><a class="btn btn-dark" href="#services">View Properties <i data-lucide="arrow-up-right"></i></a></div></article><article class="advisor-cta-card"><div class="advisor-cta-image"><img src="assets/talk-to-advisor-generated.svg" alt="Professional real estate advisor consultation" loading="lazy" decoding="async"></div><div class="advisor-cta-content"><span class="property-type">02 · Consultation</span><h3>Talk to an Advisor</h3><p>Get clear, personal real estate guidance from the SAMA United team.</p><a class="btn btn-dark" href="#contact">Talk to an Advisor <i data-lucide="user-round"></i></a></div></article></div></div>`;
